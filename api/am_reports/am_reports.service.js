...
module.exports = {
    ...
    getAllItemList: (callBack) => {
        pool.query(
            `SELECT 
            COUNT(am_item_name_creation.item_creation_slno) AS item_creation_count,
            am_item_name_creation.item_creation_slno,item_name,
            asset_spare,
            item_custodian_dept,
        ...
        OR (
            am_item_map_slno IS NULL OR                                                                                   
            am_asset_item_map_master.item_create_status = 1 
        OR (am_asset_item_map_master.item_create_status = 0 AND am_asset_item_map_master.asset_item_condmnation = 1)
            )              
		or(am_spare_item_map_slno is null OR
        am_spare_item_map_master.spare_create_status=1
                OR (am_spare_item_map_master.spare_create_status = 0 AND am_spare_item_map_master.spare_condamtn = 1)
            )              
            GROUP BY 
            am_item_name_creation.item_creation_slno
            order by
            am_item_name_creation.item_creation_slno asc `,
            [],
            (error, results, feilds) => {
                if (error) {
                    return callBack(error);
