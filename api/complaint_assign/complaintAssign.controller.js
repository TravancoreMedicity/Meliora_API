...
const {
  ...
  getEmployee,
  detailedAssign,
  getcomplaintAssignbyEmployee,
  quickAssigncompstatus,
  detailedAssigncompstatus,
  getassistantEmployee,
  insertAssistemp,
  getALLcomplaintbyEmployee,
  getIndividualassitemployee,
  AssistantRecieved,
  checkInsertVal,
  TransferDept,
  assignedListNotRectifiedOnly,
  rectifiedListForVErify,
  AssistMultiple,
  getALLAssignedComList,
  EmployeeInactive,
  beforAssignHold,
  empTransInactive,
  sendMeassageUser,
  ReadMeassageUser,
  AssistReqListAll,
  getAssistRequestEmps,
  assistTransInactive,
  AssistReqEmployee,
  AssisttransferInsert,
  SupervsrVerifyPending,
  SupervsrVerify,
  ReopenComplaintInsert,
  AskQuery,
  changeQueryStatus,
  getQuery,
  replyQuery,
  ...
  getAssistRequestCount,
  getDeptPengingTicketCount,
  getDeptEmployees,
...
module.exports = {
      ...
      if (Object.keys(value).length === 0) {
        quickAssign(body, (err, results) => {
          ...
          quickAssigncompstatus(body, (err, results) => {
            ...
            req.io.emit(
              "message",
              `New Complaint Assigned For you ! Please Check`
            );
            return res.status(200).json({
              success: 1,
  ...
  AssistMultiple: (req, res) => {
    ...
    var newList = body.map((val, index) => {
      return [
        val.complaint_slno,
        val.assigned_emp,
        val.assist_assign_date,
        val.assist_flag,
        val.assist_requested_emp,
        val.assign_rect_status,
        val.assigned_user,
      ];
    });
  ...
  EmployeeInactive: (req, res) => {
    ...
    EmployeeInactive(newList, (err, results) => {
      ...
      req.io.emit(
        "message",
        `One Complaint in your Assigned List is Transfer to another Employee`
      );
      return res.status(200).json({
        success: 1,
  ...
  getAssistRequestEmps: (req, res) => {
    ...
    getAssistRequestEmps(id, (err, results) => {
      ...
      req.io.emit(
        "message",
        `Some One Requested You For Assist ! Please Check`
      );
      return res.status(200).json({
        success: 1,
        data: results,
    ...
    SupervsrVerify(body, (err, results) => {
      ...
      if (body.verify_spervsr === 2) {
        ReopenComplaintInsert(body, (err, results) => {
          ...
          req.io.emit(
            "message",
            `New Complaint  Supervisor Not Verified ! Please Check`
          );
          return res.status(200).json({
            success: 1,
  ...
  getAssistRequestDetails: (req, res) => {
    const id = req.params.id;
    getAssistRequestDetails(id, (err, results) => {
      if (err) {
        logger.logwindow(err);
        return res.status(400).json({
          success: 2,
