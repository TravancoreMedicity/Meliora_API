...
module.exports = {
  ...
  checkUpdateVal: (data, callBack) => {
    pool.query(
      ...
      [data.emp_username, data.emp_slno],
      (error, results, feilds) => {
        if (error) {
          return callBack(error);
        }
  ...
  employeeupdate: (data, callBack) => {
    pool.query(
      `UPDATE co_employee
                ...
                WHERE emp_slno = ?`,
      [data.emp_username, data.emp_password, data.emp_status, data.emp_slno],
      (error, results, fields) => {
        if (error) {
          return callBack(error);
        }
  ...
  getemplpyee: (callBack) => {
    pool.query(`SELECT * FROM co_employee`, [], function (err, results) {
      if (err) {
        return callBack(error);
      }
      return callBack(null, results);
  ...
  changepasword: (data, callBack) => {
    pool.query(
      `UPDATE co_employee
            ...
            `,
      [data.emp_password, data.edit_user, data.emp_no],
      (error, results, feilds) => {
        if (error) {
          return callBack(error);
        }
