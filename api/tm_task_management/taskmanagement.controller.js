...
const { CreateTaskInsert, CreateTaskDetailInsert, CreateTaskView, CreateSubTaskInsert, CreateTaskSubTaskDetailInsert, SubTaskviewByid, MasterTaskviewBySecid,
    ...
    getAllGoals, 
    getAllEmpUnderdept,
    getAllCompletedTask} = require('../tm_task_management/taskmanagement.service')
...
getAllCompletedTask: (req, res) => {
  const { EmpId, From, To,empDept } = req.body;
  ...
  if (EmpId) {
    sql += " AND tm_new_task_mast_detl.tm_assigne_emp = ? ";
    queryParams.push(EmpId);
  }
  if (empDept) {
    sql += " AND tm_new_task_mast.tm_task_dept = ? ";
    queryParams.push(empDept);
  }
  

  if (From && To) {
    sql += " AND tm_complete_date BETWEEN ? AND ? ";
    queryParams.push(From, To);
  }

  sql += " GROUP BY tm_new_task_mast.tm_task_slno ORDER BY tm_complete_date DESC";

  getAllCompletedTask(sql, queryParams, (error, results) => {
